...
export async function getDashboardStats(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const [
      ...
      printQueued, printPrinting, printComplete, printError,
    ] = await Promise.all([
      ...
      prisma.permit.count({ where: { ...pf, permitType: 'IDP', dateOfExpiry: { lte: cutoff30.toISOString().slice(0, 10) }, status: { not: 'rejected' } } }),
    ...
    res.json({
      ...
      scope: scope.level === 'national' ? 'national'
           : scope.level === 'regional' ? { regionName: scope.regionName }
  ...
  try {
    ...
    const permits = await prisma.permit.findMany({
      where: {
        ...pf,
        permitType: 'IDP',
        dateOfExpiry: { gte: new Date().toISOString().slice(0, 10), lte: cutoff.toISOString().slice(0, 10) },
    ...
    res.json(permits);
  } catch (err) { res.status(500).json({ error: 'Failed to get expiring permits' }); }
}
...
export async function getRecentActivity(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    res.json(logs);
  } catch (err) { res.status(500).json({ error: 'Failed to get recent activity' }); }
}
...
export async function getPrinterStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const printerName = nameCfg?.value?.trim() || 'DVLA-PRN-001 (P4000)';
    ...
    type PrinterData = { online: boolean; ink?: { C: number; M: number; Y: number; K: number }; serial?: string };
    ...
    res.json({ configured: true, online: result.online, printerName, ink: result.ink ?? null, serial: result.serial ?? null });
  } catch (err) { res.status(500).json({ error: 'Failed to get printer status' }); }
}
