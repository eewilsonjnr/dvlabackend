...
const GHANA_REGIONS: RegionDef[] = [
  ...
  },
  {
    ...
    districts: [{ name: 'Jasikan District Office', town: 'Jasikan', address: 'Jasikan Main Road' }],
  },
  {
    name: 'Bono Region',
    town: 'Sunyani',
  ...
  },
  {
    ...
    districts: [{ name: 'Kintampo District Office', town: 'Kintampo', address: 'Kintampo Road' }],
  },
  {
    name: 'Ahafo Region',
    town: 'Goaso',
    address: 'DVLA Office, Goaso',
    districts: [{ name: 'Mim District Office', town: 'Mim', address: 'Mim Main Road' }],
  },
  {
  ...
  },
  {
    ...
    districts: [{ name: 'Bole District Office', town: 'Bole', address: 'Bole Market Road' }],
  },
  {
    name: 'North East Region',
    town: 'Nalerigu',
    address: 'DVLA Office, Nalerigu',
    districts: [{ name: 'Gambaga District Office', town: 'Gambaga', address: 'Gambaga Main Road' }],
  },
  {
...
async function main() {
  ...
  const adminRole = await prisma.role.upsert({
    ...
    create: {
      ...
      isSystem: true,
      permissions: { connect: connect(ADMIN_PERMS) },
    },
  });
  const operatorRole = await prisma.role.upsert({
    ...
    create: {
      name: 'OPERATOR',
      description: 'Day-to-day permit operations',
      isSystem: true,
      permissions: { connect: connect(OPERATOR_PERMS) },
    },
  });
  const supervisorRole = await prisma.role.upsert({
    ...
    create: {
      name: 'SUPERVISOR',
      description: 'Permit approval and quality oversight',
      isSystem: true,
      permissions: { connect: connect(SUPERVISOR_PERMS) },
    },
  });
  ...
  console.log(
    `  ✓ Regional Centres (${GHANA_REGIONS.length}) + District Offices (${totalDistricts})`,
  );
  ...
  const operatorUser = await prisma.adminUser.upsert({
    ...
    create: {
      email: 'operator@dvla.gov.gh',
      password: await bcrypt.hash('Operator@1', 10),
      firstName: 'Abena',
      lastName: 'Mensah',
      dvlaRole: 'Operator',
  ...
  await prisma.adminUser.upsert({
    ...
    create: {
      email: 'supervisor@dvla.gov.gh',
      password: await bcrypt.hash('Supervisor@1', 10),
      firstName: 'Kofi',
      lastName: 'Boateng',
      dvlaRole: 'Supervisor',
  ...
  await prisma.adminUser.upsert({
    ...
    update: { officeId: kumasiRegional.id, notificationEmail: 'paaekwaw@gmail.com' } as any,
    create: {
      email: 'operator.kumasi@dvla.gov.gh',
      password: await bcrypt.hash('Kumasi@2025', 10),
      firstName: 'Yaa',
      lastName: 'Boateng',
      dvlaRole: 'Operator',
      roleId: operatorRole.id,
  ...
  const configs = [
    {
      key: 'printer_api_endpoint',
      value: '',
      description: 'Optional direct printer REST API endpoint',
    },
    {
      key: 'printer_name',
      value: 'Surys P400',
      description: 'Exact printer name as shown in OS (used by local print agent)',
    },
    {
      key: 'printer_agent_port',
      value: '6161',
      description: 'Port the local DVLA Print Agent listens on',
    },
    {
      key: 'issuing_authority',
      value: 'DVLA Ghana',
      description: 'Issuing authority name on permits',
    },
    ...
    {
      key: 'place_of_issue_options',
      value:
        'Accra,Kumasi,Takoradi,Tamale,Cape Coast,Sunyani,Ho,Koforidua,Bolgatanga,Wa,Dambai,Nalerigu,Damongo,Sefwi Wiawso,Goaso,Techiman',
      description: 'Comma-separated DVLA regional issuing offices',
    },
  ];
  ...
  const applicants = await Promise.all([
    prisma.applicant.upsert({
      where: { nationalId: 'GHA-1001-2345' },
      update: {},
      create: {
        surname: 'ASANTE',
        otherNames: 'Kwame Boateng',
        placeOfBirth: 'Kumasi',
        dateOfBirth: isoDate(1985, 3, 12),
        homeAddress: '14 Independence Ave, Accra',
        nationalId: 'GHA-1001-2345',
        licenceNumber: 'GH-DL-001234',
      },
    }),
    prisma.applicant.upsert({
      where: { nationalId: 'GHA-1002-6789' },
      update: {},
      create: {
        surname: 'MENSAH',
        otherNames: 'Abena Efua',
        placeOfBirth: 'Cape Coast',
        dateOfBirth: isoDate(1990, 7, 24),
        homeAddress: '7 Cantonments Road, Accra',
        nationalId: 'GHA-1002-6789',
        licenceNumber: 'GH-DL-005678',
      },
    }),
    prisma.applicant.upsert({
      where: { nationalId: 'GHA-1003-1122' },
      update: {},
      create: {
        surname: 'OWUSU',
        otherNames: 'Yaw Darko',
        placeOfBirth: 'Accra',
        dateOfBirth: isoDate(1978, 11, 5),
        homeAddress: '23 Osu Oxford St, Accra',
        nationalId: 'GHA-1003-1122',
        licenceNumber: 'GH-DL-009012',
      },
    }),
    prisma.applicant.upsert({
      where: { nationalId: 'GHA-1004-3344' },
      update: {},
      create: {
        surname: 'AMPONSAH',
        otherNames: 'Akosua Nyarko',
        placeOfBirth: 'Takoradi',
        dateOfBirth: isoDate(1995, 1, 30),
        homeAddress: '5 Sekondi Road, Takoradi',
        nationalId: 'GHA-1004-3344',
        licenceNumber: 'GH-DL-012345',
      },
    }),
    prisma.applicant.upsert({
      where: { nationalId: 'GHA-1005-5566' },
      update: {},
      create: {
        surname: 'OSEI',
        otherNames: 'Nana Yaw',
        placeOfBirth: 'Sunyani',
        dateOfBirth: isoDate(1982, 6, 18),
        homeAddress: '9 Brong-Ahafo Rd, Sunyani',
        nationalId: 'GHA-1005-5566',
        licenceNumber: 'GH-DL-015678',
      },
    }),
    prisma.applicant.upsert({
      where: { nationalId: 'GHA-1006-7788' },
      update: {},
      create: {
        surname: 'ADJEI',
        otherNames: 'Maame Serwaah',
        placeOfBirth: 'Ho',
        dateOfBirth: isoDate(1993, 9, 2),
        homeAddress: '3 Volta Region Ave, Ho',
        nationalId: 'GHA-1006-7788',
        licenceNumber: 'GH-DL-018901',
      },
    }),
  ]);
  ...
  const existingP1 = await prisma.permit.findUnique({
    where: { referenceNumber: refNum('IDP', 1) },
  });
  const p1 =
    existingP1 ??
    (await prisma.permit.create({
      data: {
        permitType: 'IDP',
        referenceNumber: refNum('IDP', 1),
        applicantId: applicants[0].id,
        operatorId: operatorUser.id,
        officeId: accraRegional.id,
        status: 'issued',
        placeOfIssue: 'DVLA — Accra Regional Centre',
        classOfLicence: 'B',
        dateOfIssue: isoDate(2025, 1, 15),
        dateOfExpiry: isoDate(2026, 1, 14),
  ...
  const existingP2 = await prisma.permit.findUnique({
    where: { referenceNumber: refNum('IDP', 2) },
  });
  const p2 =
    existingP2 ??
    (await prisma.permit.create({
      data: {
        ...
        placeOfIssue: 'DVLA — Accra Regional Centre',
        classOfLicence: 'B, C',
        dateOfIssue: isoDate(2025, 2, 20),
        dateOfExpiry: isoDate(2026, 2, 19),
  ...
  if (!(await prisma.permit.findUnique({ where: { referenceNumber: refNum('IDP', 3) } }))) {
    await prisma.permit.create({
      data: {
        ...
        placeOfIssue: 'DVLA — Accra Regional Centre',
        classOfLicence: 'A',
        dateOfIssue: isoDate(2025, 3, 1),
        dateOfExpiry: isoDate(2026, 2, 28),
      },
    });
  }
  ...
  if (!(await prisma.permit.findUnique({ where: { referenceNumber: refNum('IDP', 4) } }))) {
    await prisma.permit.create({
      data: {
        ...
        placeOfIssue: 'DVLA — Accra Regional Centre',
        classOfLicence: 'B',
        dateOfIssue: isoDate(2025, 3, 5),
        dateOfExpiry: isoDate(2026, 3, 4),
        rejectionReason:
          'Biometric photo does not meet ICAO standards — resubmit with neutral expression.',
      },
    });
  }

  const existingP5 = await prisma.permit.findUnique({
    where: { referenceNumber: refNum('ICMV', 1) },
  });
  const p5 =
    existingP5 ??
    (await prisma.permit.create({
      data: {
        permitType: 'ICMV',
        referenceNumber: refNum('ICMV', 1),
        applicantId: applicants[4].id,
        operatorId: operatorUser.id,
        officeId: kumasiRegional.id,
        status: 'issued',
        placeOfIssue: 'DVLA — Kumasi Regional Centre',
        bookletNumber: 'BKL-2025-00002',
        ownerSurname: 'OSEI',
        ownerOtherNames: 'Nana Yaw',
        ownerHomeAddress: '9 Brong-Ahafo Rd, Sunyani',
        classOfVehicle: 'Saloon',
        makerOfChassis: 'Toyota',
        typeOfChassis: 'Corolla',
        serialNumber: 'TYT-20-00456',
        engineNumber: 'ENG-4A-FE-2020',
        numberOfCylinders: '4',
        horsePower: '132',
        bodyShape: 'Sedan',
        bodyColour: 'Silver',
  ...
  if (!(await prisma.permit.findUnique({ where: { referenceNumber: refNum('ICMV', 2) } }))) {
    await prisma.permit.create({
      data: {
        ...
        placeOfIssue: 'DVLA — Kumasi Regional Centre',
        ownerSurname: 'ADJEI',
        ownerOtherNames: 'Maame Serwaah',
        ownerHomeAddress: '3 Volta Region Ave, Ho',
        classOfVehicle: 'SUV',
        makerOfChassis: 'Hyundai',
        typeOfChassis: 'Tucson',
        serialNumber: 'HYU-22-00789',
        engineNumber: 'ENG-G4NA-2022',
        numberOfCylinders: '4',
        horsePower: '175',
        bodyShape: 'SUV',
        bodyColour: 'Black',
  ...
  if (!(await prisma.rfidEncoding.findFirst({ where: { permitId: p1.id } }))) {
    await prisma.rfidEncoding.create({
      data: {
        permitId: p1.id,
        printJobId: job1.id,
        status: 'encoded',
        chipSerialNumber: 'RFID-2025-001-A4F2',
        verificationResult: 'pass',
        verificationDetails: 'All ICAO DG1-DG15 groups verified.',
  ...
  if (!(await prisma.rfidEncoding.findFirst({ where: { permitId: p2.id } }))) {
    await prisma.rfidEncoding.create({
      data: { permitId: p2.id, printJobId: job2.id, status: 'pending' },
    });
  }
  if (!(await prisma.rfidEncoding.findFirst({ where: { permitId: p5.id } }))) {
    await prisma.rfidEncoding.create({
      data: {
        ...
        printJobId: job5.id,
        status: 'encoded',
        chipSerialNumber: 'RFID-2025-002-B7C9',
        verificationResult: 'pass',
        verificationDetails: 'All ICAO DG1-DG15 groups verified.',
  ...
  if (!(await prisma.qcResult.findFirst({ where: { permitId: p2.id } }))) {
    await prisma.qcResult.create({
      data: { permitId: p2.id, printJobId: job2.id, result: 'pending' },
    });
  }
    ...
    const auditEntries = [
      {
        ...
        action: 'USER_LOGIN',
        outcome: 'success',
        details: 'Admin login from Accra HQ',
      },
      {
        userId: operatorUser.id,
        operatorName: 'Abena Mensah',
        action: 'USER_LOGIN',
        outcome: 'success',
        details: 'Operator login from Accra Regional Centre',
      },
      {
        userId: operatorUser.id,
        operatorName: 'Abena Mensah',
        ...
        outcome: 'success',
        applicantRef: 'GHA-1001-2345',
        details: 'Registered applicant Kwame Asante',
      },
      {
        userId: operatorUser.id,
        operatorName: 'Abena Mensah',
        ...
        outcome: 'success',
        applicantRef: refNum('IDP', 1),
        details: 'IDP permit created at Accra Regional Centre',
      },
      {
        userId: adminUser.id,
        operatorName: 'Kwame Asante',
      ...
      },
      {
        ...
        outcome: 'success',
        applicantRef: refNum('IDP', 1),
        details: 'Print job queued for IDP-001',
      },
      {
        userId: operatorUser.id,
        operatorName: 'Abena Mensah',
        ...
        outcome: 'success',
        applicantRef: refNum('IDP', 1),
        details: 'Print completed — booklet BKL-2025-00001',
      },
      {
        userId: adminUser.id,
        operatorName: 'Kwame Asante',
        ...
        outcome: 'success',
        applicantRef: refNum('IDP', 1),
        details: 'RFID chip RFID-2025-001-A4F2 encoded and verified',
      },
      {
        userId: adminUser.id,
        operatorName: 'Kwame Asante',
        ...
        outcome: 'success',
        applicantRef: refNum('IDP', 1),
        details: 'QC passed — optical score 96.5%',
      },
      {
        userId: operatorUser.id,
        operatorName: 'Abena Mensah',
        ...
        outcome: 'warning',
        applicantRef: refNum('IDP', 4),
        details: 'Rejected — biometric photo ICAO non-compliance',
      },
    ];
    for (const entry of auditEntries) {
      await prisma.auditLog.create({
        data: { ...entry, ipAddress: '196.26.1.' + (100 + auditEntries.indexOf(entry)) },
      });
    }
  ...
  console.log(
    '    Operator   → operator@dvla.gov.gh          / Operator@1   [Accra Regional Centre]',
  );
  console.log(
    '    Supervisor → supervisor@dvla.gov.gh        / Supervisor@1 [Kumasi Regional Centre]',
  );
  console.log(
    '    Operator2  → operator.kumasi@dvla.gov.gh   / Kumasi@2025  [Kumasi Regional Centre]',
  );
  console.log('\n  DVLA Offices:');
  console.log(
    `    1 Head Office + ${GHANA_REGIONS.length} Regional Centres + ${totalDistricts} District Offices`,
  );
  console.log('\n  Demo data:');
